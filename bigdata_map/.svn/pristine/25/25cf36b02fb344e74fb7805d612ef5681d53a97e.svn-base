...
;(function($) {
  ...
  var Carousel = function( element, options ) {
    ...
    this.xRadius = (options.xRadius === null) ? $container.width()  / 2.2 : options.xRadius;
    ...
    this.rotateItem = function( itemIndex, rotation ) {
      ...
      var scale = farScale + ((1-farScale) * (sin+1) * 0.55);

      item.moveTo(
        this.xOrigin + (scale * ((Math.cos(rotation) * this.xRadius) - (item.fullWidth * 0.55))),
