...
public class CityTransportCapacityAnalysisService {
	...
	public CityTransportCapacityAnalysisView getRealTimeData(String carType) {
		return mapper.getRealTimeData(carType);
	}
	 ...
	 */
	@Transactional(propagation = Propagation.REQUIRED,rollbackFor=Exception.class)
    public void insertOrUpdate(CityTransportCapacityAnalysisView analysis) {
		String id = analysis.getId();
		CityTransportCapacityAnalysis oraAnalysis = loadById(id);
		if(oraAnalysis!=null){
			update(analysis);
		}else{
