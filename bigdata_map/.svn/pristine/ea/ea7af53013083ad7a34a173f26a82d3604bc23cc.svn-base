...
public class MyCommandLineRunner implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		Set<String> notExtends = new HashSet<String>();
		//将扩展名去掉
		for(String url : urlAndMethodSet){
			String urlNotExtends = url.replaceAll("\\.jsn:--:",":--:");
			notExtends.add(urlNotExtends);
		}

		for(String url : notExtends){
			urlAndMethodSet.add(url);
		}
								...
								try {
									String temStr = Class
											.forName(packageName + '.'
													+ className).getName();
									// 添加到classes
									Constant.METHOD_URL_SET.add(temStr);
