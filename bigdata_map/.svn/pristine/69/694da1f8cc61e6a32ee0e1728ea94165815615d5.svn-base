 ...
 **/
public class ComputeTraceFlyRate implements Standard {
    ...
    public BigdataBeidouCheckScoreRec computeScore(BigdataBeidouCompMonthCheck bigdataBeidouCompMonthCheck, String id) {
        ...
        //运营商联通率
        bigdataBeidouCheckScoreRec.setRealNum(bigdataBeidouCompMonthCheck.getTraceFlyRate());
        BigDecimal traceFlyRate = bigdataBeidouCompMonthCheck.getTraceFlyRate();
        ...
        if(standardDirection.equals("0")){
            if(traceFlyRate.compareTo(unscoreLine)>0){
                score = (traceFlyRate.subtract(unscoreLine)).divide(scoreLine.subtract(unscoreLine),2,BigDecimal.ROUND_HALF_UP).multiply(scoreNum);
            }
        }else {
            if(traceFlyRate.compareTo(unscoreLine)<0){
                score = (unscoreLine.subtract(traceFlyRate)).divide(unscoreLine.subtract(scoreLine),2,BigDecimal.ROUND_HALF_UP).multiply(scoreNum);
            }
        }
