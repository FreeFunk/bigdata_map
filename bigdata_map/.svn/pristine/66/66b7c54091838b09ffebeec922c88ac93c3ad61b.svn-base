 ...
 **/
public class ComputeLinkRate implements Standard {
    ...
    public BigdataBeidouCheckScoreRec computeScore(BigdataBeidouCompMonthCheck bigdataBeidouCompMonthCheck, String id) {
        ...
        //运营商联通率
        bigdataBeidouCheckScoreRec.setRealNum(bigdataBeidouCompMonthCheck.getLinkRate());
        BigDecimal linkRate = bigdataBeidouCompMonthCheck.getLinkRate();
        ...
        if(standardDirection.equals("0")){
            if(linkRate.compareTo(unscoreLine)>0){
                score = (linkRate.subtract(unscoreLine)).divide(scoreLine.subtract(unscoreLine),2,BigDecimal.ROUND_HALF_UP).multiply(scoreNum);
            }
        }else {
            if(linkRate.compareTo(unscoreLine)<0){
                score = (linkRate.subtract(unscoreLine)).divide(scoreLine.subtract(unscoreLine),2,BigDecimal.ROUND_HALF_UP).multiply(scoreNum);
